...
- `currentRuleCatalog` 已接入 Reg M-A 道具候选 catalog：当前规则可选池 117 个道具，包含 30 个普通携带道具、59 个 Mega Stone、28 个树果；突击背心 / 清净坠饰等未确认或不在池内的 seed 道具不再进入新配置入口。
...
5. 道具 / 招式 / 性格 catalog 可信分层
   - 道具第一批已完成：117 个当前规则候选道具进入 selector pool，未 join 到本地 Pokémon 的 Mega Stone 暂不在成员编辑中显示。
   - 下一步继续扩展 `currentRuleCatalog`：把招式、性格从 seed 示例池拆成当前规则确认池、社区候选池、示例 / 开发池。
