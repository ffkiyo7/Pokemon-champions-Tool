...
Current implementation note: item data is now split between raw seed rows and a small `currentRuleCatalog` selector pool. The selector pool includes only Sitrus Berry plus locally modeled Mega Stones while the full Reg M-A item catalog is pending; Assault Vest and Clear Amulet remain seed rows but are not marked current-rule legal.
...
5. Do not ingest or bundle official Pokemon artwork, screenshots, or official flavor/descriptive prose in the first real-data pass. Current type badges are CSS/text badges, not external image assets.
...
| Reg M-A item candidates | P1 | RotomPicks item list: https://rotompicks.com/en/items/; other community snapshots only as cross-checks until an official item catalog is located | Temporary visible item selector narrowing and manual review candidates | Medium. Community snapshots are not canonical; do not treat them as strong legality proof without cross-checking. |
...
| Icons/images/artwork | P3 / blocked for Pokemon artwork; CSS type badge accepted for current MVP | Official Pokedex images; PokeAPI sprites repo: https://github.com/PokeAPI/sprites; Pokemon Showdown type sprites; Bulbapedia/52poke type templates; Pokemon support IP guidance: https://support.pokemon.com/hc/en-us/articles/360000634094-Can-I-use-Pok%C3%A9mon-images-or-materials-; press asset terms: https://press.pokemon.com/en/Assets-Use-Terms | Visual polish only | High for Pokemon character artwork. Current type badges are project-owned CSS/text/color treatment. Pokemon character artwork remains temporary self-use via PokeAPI official-artwork URLs. |
...
- Type badges currently use a project-owned CSS/text/color capsule treatment. Local PNG type icon experiments were removed from the default UI and remain ignored local assets only.
- If external type icons are reconsidered later, treat them as a separate source/licensing review rather than part of the default MVP.
